<script lang="ts">
	...
	const columns = [
		...
		{ key: 'title', label: 'Control', sortable: true },
	...
	async function loadData() {
		...
		try {
			const result = await entities.list('CTRL');
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold">Process Controls</h1>
			<p class="text-muted-foreground">Manufacturing process control definitions</p>
		</div>
		<Button onclick={() => goto('/controls/new')}>New Control</Button>
	</div>
	...
	<div class="grid gap-4 md:grid-cols-3">
		<Card>
			<CardHeader class="pb-2">
				<CardTitle class="text-sm font-medium text-muted-foreground">Total Controls</CardTitle>
			</CardHeader>
			<CardContent>
	...
	{#if !$isProjectOpen}
		<Card>
			<CardContent class="flex h-64 items-center justify-center">
				<p class="text-muted-foreground">Open a project to view controls</p>
			</CardContent>
		</Card>
	{:else}
		<EntityTable
