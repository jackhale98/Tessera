<script lang="ts">
	...
	const columns = [
		...
		{ key: 'title', label: 'Test', sortable: true },
	...
	async function loadData() {
		...
		error = null;
		try {
			const result = await entities.list('TEST');
	...
	function handleRowClick(entity: EntityData) {
		goto(`/verification/tests/${entity.id}`);
	}
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold">Test Protocols</h1>
			<p class="text-muted-foreground">Verification and validation test procedures</p>
		</div>
		<Button onclick={() => goto('/verification/tests/new')}>New Test</Button>
	</div>

	<div class="grid gap-4 md:grid-cols-4">
		<Card>
			<CardHeader class="pb-2">
				<CardTitle class="text-sm font-medium text-muted-foreground">Total Tests</CardTitle>
			</CardHeader>
			<CardContent>
		...
		<Card>
			...
			<CardContent>
				<div class="text-2xl font-bold text-blue-400">{entitiesData.filter(e => e.status === 'approved').length}</div>
			</CardContent>
		</Card>
		<Card>
			<CardHeader class="pb-2">
	...
	{#if !$isProjectOpen}
		<Card>
			<CardContent class="flex h-64 items-center justify-center">
				<p class="text-muted-foreground">Open a project to view tests</p>
			</CardContent>
		</Card>
	{:else}
		<EntityTable {columns} entities={entitiesData} {loading} searchPlaceholder="Search tests..." onRowClick={handleRowClick} />
	{/if}
</div>
