<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			const newId = await entities.save('TEST', data);
			await refreshProject();
			goto(`/verification/tests/${newId}`);
		} catch (e) {
...
<div class="space-y-6">
	<div class="space-y-4">
		<Button variant="ghost" size="sm" onclick={() => goto('/verification/tests')}>
			<ArrowLeft class="mr-2 h-4 w-4" />Back to Tests
		</Button>
		<div>
			<h1 class="text-2xl font-bold">New Test Protocol</h1>
			<p class="text-muted-foreground">Define a verification or validation test procedure</p>
		</div>
	</div>
				...
				<Card>
					<CardContent class="pt-6">
						<div class="flex flex-col gap-2">
							...
							<Button type="button" variant="outline" onclick={() => goto('/verification/tests')}>Cancel</Button>
						</div>
					</CardContent>
				</Card>
			</div>
