<script lang="ts">
	...
	const columns = [
		...
		{ key: 'title', label: 'Result', sortable: true },
	...
	async function loadData() {
		...
		error = null;
		try {
			const result = await entities.list('RSLT');
	...
	function handleRowClick(entity: EntityData) {
		goto(`/verification/results/${entity.id}`);
	}
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-muted-foreground">Test execution records and verdicts</p>
		</div>
		<Button onclick={() => goto('/verification/results/new')}>New Result</Button>
	</div>

	<div class="grid gap-4 md:grid-cols-3">
		<Card>
			<CardHeader class="pb-2">
				<CardTitle class="text-sm font-medium text-muted-foreground">Total Results</CardTitle>
			</CardHeader>
			<CardContent>
	...
	{#if !$isProjectOpen}
		<Card>
			<CardContent class="flex h-64 items-center justify-center">
				<p class="text-muted-foreground">Open a project to view results</p>
			</CardContent>
		</Card>
	{:else}
		<EntityTable {columns} entities={entitiesData} {loading} searchPlaceholder="Search results..." onRowClick={handleRowClick} />
	{/if}
</div>
