<script lang="ts">
	...
	async function loadData() {
		...
		error = null;
		try {
			const result = await entities.list('LOT');
...
<div class="space-y-6">
	...
	<div class="grid gap-4 md:grid-cols-3">
		<Card>
			<CardHeader class="pb-2">
				<CardTitle class="text-sm font-medium text-muted-foreground">Total Lots</CardTitle>
			</CardHeader>
			<CardContent>
		...
		<Card>
			...
			<CardContent>
				<div class="text-2xl font-bold text-blue-400">{entitiesData.filter(e => e.status === 'draft').length}</div>
			</CardContent>
		</Card>
		<Card>
			<CardHeader class="pb-2">
	...
	{:else}
		<EntityTable {columns} entities={entitiesData} {loading} searchPlaceholder="Search lots..." onRowClick={handleRowClick} />
	{/if}
</div>
